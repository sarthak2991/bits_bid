...
const [currentbid,setCurrent] = useState('')
  useEffect(()=>{axios.get("https://big-omc1.onrender.com/api/v1/products/"+productId,{headers : {'Authorization': `Bearer ${token}`}}).then(
    (res)=>{
      const bids = []
...
const handleClick = ()=>{
  axios.post("https://big-omc1.onrender.com/api/v1/bids/",{bidAmount,productId},{headers : {'Authorization': `Bearer ${token}`}}).then((res)=>{
    if(res.data.hasOwnProperty('result')){
...
const handleUpdate = ()=>{
  axios.get("https://big-omc1.onrender.com/api/v1/users/profile",{headers : {'Authorization': `Bearer ${token}`}}).then((res)=>{
  ...
  axios.put("https://big-omc1.onrender.com/api/v1/bids/"+bidId,{bidAmount,productId},{headers : {'Authorization': `Bearer ${token}`}}).then(
    (res)=>{
